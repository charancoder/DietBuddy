...
def diet_bot():
    ...
    title_label = ttk.Label(main_frame, text="Welcome to DietBot!", font=("Arial", 16, "bold"))
    title_label.pack(pady=10)

    # Create the input entry and label
    input_label = ttk.Label(main_frame, text="Enter your health issue:")
    input_label.pack(pady=5)

    place_holder = Frame(main_frame)
    place_holder.pack(pady=5)

    input_entry = ttk.Entry(place_holder, width=30)
    input_entry.grid(row=0, column=0, padx=5)

    mike_button = Button(place_holder, image=mike_button_image, borderwidth=0, command=speech)
    ...
    place_holder2 = Frame(main_frame)
    place_holder2.pack(pady=5)

    sound_button = Button(place_holder2, image=sound_button_image, borderwidth=0, command=speak_aloud)
